...
export default function FeaturedAuthorsStrip() {
  const featured = mockAuthors?.filter((a) => a?.featured);
              ...
              <div className="story-card p-6 group cursor-pointer hover:border-primary/30">
                <div className="flex items-center gap-4 mb-4">
                  <div className="relative w-16 h-16 rounded-full overflow-hidden flex-shrink-0 ring-2 ring-primary/20 group-hover:ring-primary/50 transition-all">
                    <AppImage
                      src={author?.avatar}
                      alt={`Foto de perfil de ${author?.name}, autora ${author?.country} de literatura romántica`}
                      fill
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                      ...
                      <span>{(author?.followers / 1000)?.toFixed(1)}k seguidoras</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-1 mt-3">
                  {author?.tags?.slice(0, 3)?.map((tag) => (
                    <span key={`featured-author-tag-${author?.id}-${tag}`} className="tag-pill-rose tag-pill text-[10px]">
                      #{tag}
                    </span>
                  ))}
                </div>
