...
export default function AuthClient() {
  ...
  // Debug state — shown on screen so you don't need DevTools
  ...
  const onRegisterValidationError = (errors: any) => {
    // react-hook-form calls this when validation blocks the submit
  ...
  const onRegisterSubmit = async (data: RegisterForm) => {
    // ── Checkpoint 1: form reached handler ──────────────────────────────────
    console.log('SIGNUP_START', { email: data.email, name: data.name });
    console.log('SUPABASE_URL', process.env.NEXT_PUBLIC_SUPABASE_URL);
    console.log('SUPABASE_KEY_PREFIX', process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY?.slice(0, 30) + '...');
    ...
    // ── Checkpoint 2: calling Supabase directly — no AuthContext wrapper ──────
    const supabase = createClient();
    ...
    // ── Checkpoint 3: full result logged ─────────────────────────────────────
    ...
    if (result.error) {
      ...
      console.error('SIGNUP_ERROR', raw);
    ...
    if (result.data?.user) {
      if (result.data.session) {
        setDebugInfo(`OK: usuario creado con sesión inmediata (id=${result.data.user.id})`);
        toast.success('¡Cuenta creada! Bienvenida a SoloLatinas.');
        router.replace(redirectTo);
        router.refresh();
      } else {
        setDebugInfo(`OK: usuario creado, confirmación de email pendiente (id=${result.data.user.id})`);
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="flex-1 flex flex-col justify-center px-6 sm:px-10 lg:px-12 xl:px-16 py-10 overflow-y-auto">
        ...
        <div className="max-w-sm w-full mx-auto">
          ...
          {/* ── Debug panel — always visible when debugInfo is set ── */}
          {debugInfo && (
...
function translateSupabaseError(msg: string): string {
  ...
  if (msg.includes('Email rate limit exceeded')) return 'Límite de correos alcanzado. Intenta en unos minutos';
  if (msg.includes('signup_disabled')) return 'El registro está temporalmente deshabilitado';
  if (msg.includes('over_email_send_rate_limit')) return 'Demasiados correos enviados. Espera unos minutos';
  return msg;
}
...
export default function AuthClient() {
  ...
  // Log active Supabase project for debugging
  if (typeof window !== 'undefined') {
    const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
    const ref = url ? url.replace('https://', '').split('.')[0] : 'unknown';
    console.log('[Supabase] Active project ref:', ref, '| URL:', url);
  }
  ...
  const onLoginSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      const msg = translateSupabaseError(error?.message || 'Error al iniciar sesión');
  ...
  const onRegisterSubmit = async (data: RegisterForm) => {
    ...
    try {
      ...
      console.log('[Auth] SignUp result:', { user: result?.user?.id, session: !!result?.session });
      ...
      const msg = translateSupabaseError(error?.message || 'Error al crear la cuenta');
      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="max-w-md w-full">
          <div className="bg-surface-elevated border border-primary/20 rounded-2xl p-8 text-center">
            ...
            <button
              onClick={() => { setAwaitingConfirmation(false); setTab('login'); }}
              className="btn-outline w-full justify-center"
            >
              Volver al inicio de sesión
            </button>
            ...
            <form onSubmit={loginForm.handleSubmit(onLoginSubmit)} className="space-y-5">
              ...
              >
                {isLoading ? (
                  ...
                  <><BookOpen size={16} /> Iniciar Sesión</>
                )}
              </button>
